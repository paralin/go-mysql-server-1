...
func resolveSystemOrUserVariable(ctx *sql.Context, a *Analyzer, col column) (sql.Expression, bool, error) {
	...
	var err error
	if col.Table() != "" {
		varName, scope, err = sqlparser.GetSetScope(col.Table(), col.Name())
		if err != nil {
			return nil, false, err
		}
	} else {
		varName, scope, err = sqlparser.GetSetScope(col.Name())
		if err != nil {
			return nil, false, err
		}
	}
	...
	case sqlparser.SetScope_Persist:
		return nil, false, fmt.Errorf("PERSIST not yet supported")
	case sqlparser.SetScope_PersistOnly:
		return nil, false, fmt.Errorf("PERSIST_ONLY not yet supported")
